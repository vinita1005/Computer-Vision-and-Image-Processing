...
def convolve2d(img, kernel):
    ...
    for m in range(len(img)):
        for n in range(len(img[0])):
            total = 0
            sublist = utils.crop(padded_img, m, m+3, n, n+3)
            mul = utils.elementwise_mul(sublist, flipped_kernel)
            for s in range(len(flipped_kernel)):
                total = total + sum(mul[s])
            img_conv[m][n] = total
...
def normalize(img):
    ...
    if((img.max() - img.min())!=0):
        normalized_img = np.divide((img - img.min()),(img.max() - img.min()))
...
def main():
    ...
    print("Determining edge magnitude")
